...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    private void init() {
        ...
        /*
         * 方法开始到分割线1之间的代码。
         * 这段代码主要用于检测 ConsumerConfig 实例是否存在，如不存在则创建一个新的实例，
        ...
        /*
         * 分割线1到分割线2之间的逻辑。
         * 这段逻辑用于从系统属性或配置文件中加载与接口名相对应的配置，并将解析结果赋值给 url 字段。
         * url 字段的作用一般是用于点对点调用。
         */
        // -------------------------------✨ 分割线2 ✨------------------------------
        ...
        /*
         * 分割线2和分割线3之间的代码用于检测几个核心配置类是否为空，为空则尝试从其他配置类中获取。
         */
        // -------------------------------✨ 分割线3 ✨------------------------------
        ...
        /*
         * 分割线3与分割线4之间的代码主要用于收集各种配置，并将配置存储到 map 中。
         */
        // -------------------------------✨ 分割线4 ✨------------------------------
    ...
    /**
     * 从字面意思上来看，createProxy 似乎只是用于创建代理对象的。
     * 但实际上并非如此，该方法还会调用其他方法构建以及合并 Invoker 实例。
     *
     * 首先根据配置检查是否为本地调用，
     * 若是，则调用 InjvmProtocol 的 refer 方法生成 InjvmInvoker 实例。
     * 若不是，则读取直连配置项，或注册中心 url，并将读取到的 url 存储到 urls 中。
     *
     * 然后根据 urls 元素数量进行后续操作。
     * 若 urls 元素数量为 1，则直接通过 Protocol 自适应拓展类构建 Invoker 实例接口。
     * 若 urls 元素数量大于 1，即存在多个注册中心或服务直连 url，此时先根据 url 构建 Invoker。
